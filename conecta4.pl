...
columnaLlena(L):- length(L,LEN), tamFila(LEN), imprimir_msg('La columna está llena').
imprimir_msg(M):- write(M), nl.

tablero_lleno(TAB, 1, ROWS):- elemAtPos(1, TAB, COLX),             % Comprobamos que las columnas del tablero tengan longitud de fila. Si es así en todas, está lleno.
                              length(COLX, ROWSX),
                              (ROWS =:= ROWSX),
                              imprimir_msg('EMPATE. El tablero está lleno').
